...
annual_capacity = df.groupby('Year')['zero-carbon estimated normalized capacity [Nm³ H₂/hour]'].sum().reset_index()

X = annual_capacity['Year'].values.reshape(-1, 1)
y = annual_capacity['zero-carbon estimated normalized capacity [Nm³ H₂/hour]'].values
...
plt.plot(annual_capacity['Year'], annual_capacity['zero-carbon estimated normalized capacity [Nm³ H₂/hour]'], label='Historical')
...
plt.title('Share of Renewable-based Projects')
plt.xlabel('Year')
plt.ylabel('Share')
plt.legend()
plt.show()
